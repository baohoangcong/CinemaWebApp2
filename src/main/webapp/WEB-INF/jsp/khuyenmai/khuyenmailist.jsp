...
<body class="d-flex flex-row justify-content-center">
	...
	<div
		...
		<div class="container">
			<div align="center">
				<h2 class=" text-uppercase fw-bold" style="font-size: 50px;">Danh sách khuyến mãi</h2>
				<div>
					<div>
						<div align="left">
							<a href="${pageContext.request.contextPath}/admin/khuyenmai/them"
								class="btn" style="margin-bottom: -120px; font-size: 30px;">Thêm khuyến mãi</a>
						</div>

						<p>${messageUpdate}</p>
						<form
							action="${pageContext.request.contextPath}/khuyenmai/timkiem"
							class="d-flex justify-content-end">
							<input type="text" name="keySearch" class="form-control"
								style="width: 15%;height: 60px" placeholder="Tìm kiếm..."> <input
								type="submit" value="Tìm kiếm" style="font-size: 30px" class="btn btn-warning">
						</form>
					</div>
				</div>
				<br>
					...
					<tbody>
						<c:forEach var="abc" items="${khuyenmais}" varStatus="status">
							...
							<tr>
								...
								<td style="padding-top: 1%">${abc.tiLeKhuyenMai}</td>
								<td><a
									...
									onclick="return confirm('Bạn có muốn xóa không?')" class="btn" style="margin-bottom: -10%">XÓA</a></td>
							</tr>
						</c:forEach>
					</tbody>
