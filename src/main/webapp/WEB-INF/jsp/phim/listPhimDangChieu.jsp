	...
	<div class="main-body"
		...
		<div>
			<form class="d-flex"
				...
				method="get">
				<input type="search" aria-label="Search" aria-describedby="search-addon" name="searchKey" class="form-control" placeholder="Tên Phim">
				 <input
					type="submit" value="Search" class="btn mx-2" id="button">
			</form>
				<br>
				<div align="center" style="color: red;">${msg2}</div>
		</div>
		...
		<div class="pagination">
			<c:if test="${currentPage > 1}">
				<a href="listPhimDangChieu?page=${currentPage-1}" class="btn btn-light btn-direction">&laquo;
					Previous</a>
			</c:if>
				...
				<c:choose>
					...
					<c:otherwise>
						<c:if
							...
							<a href="listPhimDangChieu?page=${i}">${i}</a>
						</c:if>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			...
			<c:if test="${currentPage lt totalPages}">
				<a href="listPhimDangChieu?page=${currentPage+1}" class="btn btn-light btn-direction">Next
					&raquo;</a>
			</c:if>
		</div>
