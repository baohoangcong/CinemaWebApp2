	...
	<div class="main-body"
		...
		<div>
			<form class="d-flex"
				...
				method="get">
				<input type="search" value="${searchKey}" name="searchKey" class="form-control" placeholder="Tên Phim"> <input
...
<div align="center" style="color: red;">${msg1}</div>
		<div class="pagination">
			<c:if test="${currentPage > 1}">
				<a href="searchPhimSapChieu?page=${currentPage-1}&searchKey=${searchKey}" class="btn btn-light btn-direction">&laquo;
					Previous</a>
			</c:if>
				...
				<c:choose>
					...
					<c:otherwise>
						<c:if
							...
							<a href="searchPhimSapChieu?page=${i}&searchKey=${searchKey}">${i}</a>
						</c:if>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			...
			<c:if test="${currentPage lt totalPages}">
				<a href="searchPhimSapChieu?page=${currentPage+1}&searchKey=${searchKey}" class="btn btn-light btn-direction">Next
					&raquo;</a>
			</c:if>
		</div>
