...
public class AdminController {
	...
	/*
	 * Project: Cinema WebApp Team: 2 Author : Ducnm74 Method: Thống kê thông tin
	 * khách hàng
	 */
	...
	/*
	 * Project: Cinema WebApp Team: 2 Author : Ducnm74 Method: Thống kê doanh thu
	 * rạp chiếu phim theo thời gian (màn hình chính)
	 */
	...
	/*
	 * Project: Cinema WebApp Team: 2 Author : Ducnm74 Method: Thống kê doanh thu
	 * rạp chiếu phim theo năm tùy chọn
	 */
	...
	/**
	 * Project: Cinema WebApp Team: 2 Author : ViTM Function : phan trang hien thi
	 * list danh sach cua khach Hang ben admin
	 */
	...
	/**
	 * Project: Cinema WebApp Team: 2 Author : ViTM Function : tim kiem khách hàng
	 * có trong list theo Tên
	 */
	...
	/**
	 * Project: Cinema WebApp Team: 2 Author : ViTM Function : lấy mã khách rồi chạy
	 * vào file jsp chuyền modelattribute
	 */
	...
	/**
	 * Project: Cinema WebApp Team: 2 Author : ViTM Function : từ bên jsp
	 * admin/update khi nhấn nút submit thì sẽ dẫn tới controller này và
	 * ModelAttribute sẽ hứng các dữ liệu và check validate và cập nhập vao database
	 */
	@PostMapping("/update")
	...
	/**
	 * Project: Cinema WebApp Team: 2 Author : ViTM Function : hiển thị list bằng
	 * cách lấy maKhachHang rồi chuyền lại cho Attribute và trong Repository
	 * SuDungDichVu sẽ seclect các dịch vụ của khách hàng
	 */
	...
	/**
	 * Project: Cinema WebApp Team: 2 Author : ViTM Function : hiển thị list bằng
	 * cách lấy maKhachHang rồi chuyền lại cho Attribute và trong Repository Ve sẽ
	 * seclect lịch sử mua ve của cách Hàng
	 */
	@RequestMapping({ "/listmuave/{maKhachHang}" })
	public String danhsachmuave(@PathVariable("maKhachHang") int maKhachHang, Model model,
			@RequestParam(defaultValue = "1") Integer page) {
		PageAble pageAble = new PageAble(page);
		List<Ve> ve = (List<Ve>) this.veService.findWithPageAbleall(pageAble, maKhachHang);
	...
	/**
	 * Project: Cinema WebApp Team: 2 Author : ViTM Function : xóa khách hàng khỏi
	 * danh sach , chi xóa những khách hàng chưa dùng dùng vụ nào
	 */
	...
	public String delete(@PathVariable(name = "maKhachHang") int maKhachHang, Model model,
		...
		if (suDungDichVuService.findKhachHang(maKhachHang).size() == 0) {
			...
			PageAble pageAble = new PageAble((int) page);
			List<KhachHang> khachHang = (List<KhachHang>) khachHangService.findWithPageAble(pageAble);
			model.addAttribute("khachHang", khachHang);
			model.addAttribute("totalPages", khachHangService.totalPages(pageAble));
			model.addAttribute("currentPage", page);
			model.addAttribute("thanhcong", "\u0110\u00e3 x\u00f3a kh\u00e1ch h\u00e0ng c\u00f3 m\u00e3 l\u00e0 : ");
		...
		model.addAttribute("totalPages", khachHangService.totalPages(pageAble));
		model.addAttribute("currentPage", page);
		model.addAttribute("thatbai", "Kh\u00f4ng th\u1ec3 x\u00f3a kh\u00e1ch h\u00e0ng c\u00f3 m\u00e3 l\u00e0 : ");
		model.addAttribute("maKhachHang", maKhachHang);
		return "/admin/deletethatbai";
	}
