...
public class KhachHangController {
	...
	public String search(@RequestParam(name = "searchKey", required = false) String searchKey,
		...
		List<Phim> listPhim1 = phimService.findTop7();
	...
	/**
	 * Trang hiển thị chi tiết một bộ phim
	 * Proeject: Cinema WebApp
	 * Team: 2
	 * Dang Ngoc Sinh
	 * 30/08/1998
	...
	public String viewDangKy(@PathVariable String id, Model model) {
		...
		List<Phim> listPhim = phimService.findTop3Next();
		...
		if (suatChieus.size() == 0) {
			model.addAttribute("suatChieuMessage", "Hiện tại chưa có suất chiếu cho phim vào ngày này, vui lòng chọn lại ngày!");
		}
...
//		List<SuatChieu> suatChieus = suatChieuService.searchByDate(phim1.getMaPhim(), ngayChieu.getNgayChieu());
		List<SuatChieu> suatChieus = suatChieuService.getByCurrentDate(phim1.getMaPhim());
		if (suatChieus.size() == 0) {
			model.addAttribute("suatChieuMessage", "Hiện tại chưa có suất chiếu cho phim vào ngày này, vui lòng chọn lại ngày!");
			model.addAttribute("NgayChieu", new NgayChieu());
//			model.addAttribute("NgayChieu", ngayChieu);
			model.addAttribute("suatChieus", suatChieus);
			model.addAttribute("listPhim", phimService.findTop3Next());
			model.addAttribute("phim", phim);
			return "/phim/phimview";
		}
		model.addAttribute("NgayChieu", new NgayChieu());
