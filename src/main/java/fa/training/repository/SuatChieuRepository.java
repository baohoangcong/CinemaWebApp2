...
public class SuatChieuRepository {
	...
	public List<SuatChieu> getByCurrentDate(String maPhim){
		Session session = sessionFactory.getCurrentSession();
		Query<SuatChieu> query = session.createQuery("SELECT c FROM SuatChieu c WHERE c.phim.maPhim = :maPhim and c.ngayChieu >= :ngayHienTai", SuatChieu.class);
		query.setParameter("maPhim", maPhim);
		query.setParameter("ngayHienTai", LocalDate.now());
		return query.getResultList();
	}
}
